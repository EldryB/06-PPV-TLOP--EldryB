...
class Room:
    ...
    def _generate_objects(self) -> None:
        ...
        def _find_free_rect(width: int, height: int,
                            min_x: int, max_x: int,
                            min_y: int, max_y: int,
                            max_attempts: int = 50) -> Optional[pygame.Rect]:
            for _ in range(max_attempts):
                x = random.randint(min_x, max(min_x, max_x - width))
                y = random.randint(min_y, max(min_y, max_y - height))
                candidate = pygame.Rect(x, y, width, height)
                if not any(candidate.colliderect(r) for r in occupied):
                    occupied.append(candidate)
                    return candidate
            return None

        if not hasattr(self.player, 'chest_spawned'):
        ...
        if not self.player.chest_spawned and not getattr(self.player, 'has_bow', False):
            if random.randint(1, 10) <= 2:   # 20% probability per room
                ...
                rect = _find_free_rect(
                    ...
                    max_x=settings.MAP_RENDER_OFFSET_X + (settings.MAP_WIDTH - 3) * settings.TILE_SIZE,
                    min_y=settings.MAP_RENDER_OFFSET_Y + settings.TILE_SIZE * 2,
                    max_y=settings.MAP_RENDER_OFFSET_Y + (settings.MAP_HEIGHT - 3) * settings.TILE_SIZE,
                )
                if rect:
                    chest = GameObject(chest_def, rect.x, rect.y)
                    self.objects.append(chest)
                    self.player.chest_spawned = True

        switch_def = GAME_OBJECT_DEFS["switch"]
        switch_rect = _find_free_rect(
            switch_def["width"], switch_def["height"],
            min_x=settings.MAP_RENDER_OFFSET_X + settings.TILE_SIZE,
            max_x=settings.VIRTUAL_WIDTH - settings.TILE_SIZE * 2,
            min_y=settings.MAP_RENDER_OFFSET_Y + settings.TILE_SIZE,
            max_y=settings.MAP_HEIGHT * settings.TILE_SIZE + settings.MAP_RENDER_OFFSET_Y - settings.TILE_SIZE,
        )

        if switch_rect is None:
            switch_rect = pygame.Rect(
                settings.MAP_RENDER_OFFSET_X + settings.TILE_SIZE,
                settings.MAP_RENDER_OFFSET_Y + settings.TILE_SIZE,
                switch_def["width"], switch_def["height"],
            )
        switch = GameObject(switch_def, switch_rect.x, switch_rect.y)
        ...
        pot_def = GAME_OBJECT_DEFS["pot"]
        for y in range(2, self.height):
            for x in range(2, self.width):
                if random.randint(1, 20) == 1:
                    candidate = pygame.Rect(
                        x * settings.TILE_SIZE, y * settings.TILE_SIZE,
                        pot_def["width"], pot_def["height"],
                    )
                    if not any(candidate.colliderect(r) for r in occupied):
                        occupied.append(candidate)
                        self.objects.append(GameObject(pot_def, candidate.x, candidate.y))
    ...
    def open_adjacent_chest(self, player) -> bool:
